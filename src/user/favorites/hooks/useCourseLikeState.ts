...
export const useCourseLikeState = ({
  ...
  useFavoritedModal(
    adaptValueWithCallbacksAsVariableStrategyProps(showingLikedUntil),
    modals
  );

  const showingUnlikedUntil = useWritableValueWithCallbacks<number | undefined>(
    () => undefined
  );
  useUnfavoritedModal(
    adaptValueWithCallbacksAsVariableStrategyProps(showingUnlikedUntil),
    modals
  );
  ...
  const handleCancelableStart = useCallback(
    async (
      ...
      if (state.finishing) {
        ...
        return {
          signal: undefined,
          loginContext: null as any,
          courseRef: null as any,
        };
      }
    ...
    (): CancelablePromise<{ uid: string; value: Date | null }> =>
      constructCancelablePromise({
        body: async (state, resolve, reject) => {
          const { signal, loginContext, courseRef } =
            await handleCancelableStart(state, reject);
          if (state.finishing) {
            state.done = true;
            reject(new Error('canceled'));
            return;
          ...
          try {
            response = await apiFetch(
              '/api/1/courses/search_public?course_jwt=' +
                encodeURIComponent(courseRef.jwt),
              {
                method: 'POST',
      ...
      constructCancelablePromise({
        body: async (state, resolve, reject) => {
          ...
          try {
            ...
            setVWC(likedAtVWC, {
              uid: courseRef.uid,
              value: new Date(data.liked_at * 1000),
            });
