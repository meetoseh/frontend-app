...
const getDataFromRef = (
  ...
  ref: ExpirableCourseRef
) => CancelablePromise<Result<CourseLikeState>>) => {
  return (ref) => {
...
export const createCourseLikeState = ({
  ...
  const likedAtVWC = createWritableValueWithCallbacks<Date | null | undefined>(
    undefined
  );
  const showLikedUntilVWC = createWritableValueWithCallbacks<
    number | undefined
  >(undefined);
  ...
  const refresh = (): CancelablePromise<void> => {
    return constructCancelablePromise({
      body: async (state, resolve, reject) => {
        ...
        if (state.finishing) {
          throw new Error(
            'unexpected javascript engine yield leading to integrity error'
          );
        }
        ...
        try {
          ...
          try {
            response = await apiFetch(
              '/api/1/courses/search_public?course_jwt=' +
                encodeURIComponent(course.jwt),
              {
                method: 'POST',
                headers: {
          ...
          const series = convertUsingMapper(
            data.items[0],
          ...
          if (state.finishing) {
            state.done = true;
            reject(
              new DisplayableError('canceled', 'refresh series like state')
            );
            return;
          }
      ...
      body: async (state, resolve, reject) => {
        ...
        try {
          ...
          const err =
            e instanceof DisplayableError
              ? e
              : new DisplayableError('client', 'like', `${e}`);
      ...
      body: async (state, resolve, reject) => {
        ...
        try {
          ...
          const err =
            e instanceof DisplayableError
              ? e
              : new DisplayableError('client', 'unlike', `${e}`);
