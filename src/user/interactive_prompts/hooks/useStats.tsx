...
export const useStats = ({
  prompt,
  promptTime,
}: UseStatsProps): PromptStats => {
  const stats = useRef<Stats>() as MutableRefObject<Stats>;
  const onStatsChanged = useRef<
    Callbacks<StatsChangedEvent>
  >() as MutableRefObject<Callbacks<StatsChangedEvent>>;
  ...
  useEffect(() => {
    ...
    async function handleStats() {
      ...
      const bins: Map<
        number,
        { loaded: true; stats: Stats } | { loaded: false }
      > = new Map();
      ...
      while (active) {
        ...
        const desiredBins = getDesiredBins(
        ...
        const easeFromBin = getEaseFromBin(
          prompt.durationSeconds * 1000,
          binWidth,
          now
        );
        const easeToBin = getEaseToBin(
          prompt.durationSeconds * 1000,
          binWidth,
          now
        );
        ...
        if (easeFromRaw.loaded && easeToRaw.loaded) {
          const binStartsAt = getBinStart(
            prompt.durationSeconds * 1000,
            binWidth,
            easeToBin
          );
          const binEndsAt = getBinEnd(
            prompt.durationSeconds * 1000,
            binWidth,
            easeToBin
          );
      ...
      try {
        ...
        if (attemptN < 3) {
          await new Promise((resolve) =>
            setTimeout(resolve, 1000 * 2 ** attemptN)
          );
 ...
 */
export const waitUntilNextStatsUpdate = (
  stats: PromptStats
): Promise<StatsChangedEvent> => {
...
const interpolate = (
  ...
  const users = Math.round(
    (from?.users ?? 0) + ((to?.users ?? 0) - (from?.users ?? 0)) * progress
  );
  const likes = Math.round(
    (from?.likes ?? 0) + ((to?.likes ?? 0) - (from?.likes ?? 0)) * progress
  );
  const press =
    from?.press || to?.press
      ? Math.round(
          (from?.press ?? 0) +
            ((to?.press ?? 0) - (from?.press ?? 0)) * progress
        )
      : null;
  const pressActive =
    from?.pressActive || to?.pressActive
      ? Math.round(
          (from?.pressActive ?? 0) +
            ((to?.pressActive ?? 0) - (from?.pressActive ?? 0)) * progress
        )
      : null;
  ...
  if (from?.numericActive || to?.numericActive) {
    ...
    if (to?.numericActive) {
      ...
      while (!next.done) {
        ...
        numericActive.set(
          key,
          Math.round(fromValue + (value - fromValue) * progress)
        );
        next = iter.next();
      }
    }
  ...
  return {
    promptTime:
      (from?.promptTime ?? 0) +
      ((to?.promptTime ?? 0) - (from?.promptTime ?? 0)) * progress,
...
const getDesiredBins = (
  promptDuration: number,
  binWidth: number,
  promptTime: number
): number[] => {
  const totalNumberOfBins = Math.ceil(promptDuration / binWidth);
  const firstBinToInclude = getEaseFromBin(
    promptDuration,
    binWidth,
    promptTime
  );
