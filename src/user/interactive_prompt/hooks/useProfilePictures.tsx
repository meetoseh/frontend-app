  ...
  useValueWithCallbacksEffect(
    ...
    useCallback(
      (loginRaw) => {
        ...
        const imageRequestsByPromptTime = new Map<
          number,
          OsehImageRequestedState[]
        >();
        ...
        async function getImageRefs() {
          ...
          while (active && nextBinToLoad <= lastBin) {
            ...
            if (nextBinToLoad > currentBin + 1) {
              ...
              const cancelPromise =
                createCancelablePromiseFromCallbacks(cancelers);
              await Promise.race([
            ...
            try {
              ...
              const data: { items: { picture: OsehImageRef }[] } =
                await response.json();
            ...
            for (const imgReq of imageRequests) {
              ...
              if (!state.loading) {
                ...
                if (imageLoadedPromise === null) {
                  imageLoadedPromise =
                    createCancelablePromiseFromCallbacks(imageLoadedCallbacks);
                }
                (() => {
                  const handler = () => {
      ...
      [promptVWC, promptTimeVWC, statsVWC, imagesHandler, result]
    )
  );
