...
export const makeDaysOfWeekPretty = (daysOfWeek: DayOfWeek[]): string => {
  ...
  daysOfWeek = [...daysOfWeek].sort((a, b) => {
    return (
      printedDaysOfWeekOrder.indexOf(a) - printedDaysOfWeekOrder.indexOf(b)
    );
  });
  ...
  if (daysOfWeek.length === 3) {
    return `${daysOfWeek[0].slice(0, 3)}, ${daysOfWeek[1].slice(
      0,
      3
 ...
 */
export const makeSecondsOffsetPretty = (
  seconds: number,
  opts?: SecondsOffsetFormatOptions
): string => {
