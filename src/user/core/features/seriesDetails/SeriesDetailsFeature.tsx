...
> = {
  ...
  useResources(state, required, allStates) {
    ...
    const journeysVWC = useWritableValueWithCallbacks<
      MinimalCourseJourney[] | null | undefined
    >(() => undefined);
    ...
    useValuesWithCallbacksEffect([courseVWC, loginContextRaw.value], () => {
      ...
      async function fetchJourneysInner(signal: AbortSignal | undefined) {
        ...
        if (!response.ok) {
          console.log('error loading course journeys:', response.status);
          throw response;
        }
        signal?.throwIfAborted();
  ...
  component: (state, resources) => (
    <SeriesDetails state={state} resources={resources} />
  ),
};
