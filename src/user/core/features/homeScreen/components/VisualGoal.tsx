  ...
  return [
    {
      ...
      largeArcFlag:
        getAngleDistanceCW(path.startAngleDegrees, cutAngle) > 180 ? 1 : 0,
    },
    {
      ...path,
      ...
      start: cutPosition,
      largeArcFlag:
        getAngleDistanceCW(cutAngle, path.endAngleDegrees) > 180 ? 1 : 0,
    },
  ];
};
 ...
 * day of their goal.
 */
export const VisualGoal = ({
  state: stateVWC,
}: {
  state: ValueWithCallbacks<VisualGoalState>;
}) => {
  const radius = 45;
  const strokeWidth = 6;
  const viewBox = {
  ...
  const pathInfoVWC = useMappedValueWithCallbacks(goalVWC, (goal) => {
    ...
    const remainingAngleDegrees =
      360 - cutAngleDegrees - (goal - 1) * spacerAngleDegrees;
    ...
    for (let idx = 0; idx < goal; idx++) {
      ...
      const largeArcFlag =
        getAngleDistanceCW(startAngleStd, endAngleStd) > 180 ? 1 : 0;
      const sweepFlag = 1;
      ...
      <RenderGuardedComponent
        ...
        component={({ filled, segments, skipFilled }) => {
          ...
          for (let idx = 0; idx < segments.length; idx++) {
            ...
            paths.push(
              <Path
                d={`M${svgn(segment.start.x)} ${svgn(segment.start.y)} A${svgn(
                  radius
                )} ${svgn(radius)} 0 ${svgn(segment.largeArcFlag)} ${svgn(
                  segment.sweepFlag
                )} ${svgn(segment.end.x)} ${svgn(segment.end.y)}`}
          ...
          for (let subidx = 0; subidx < toDraw.length; subidx++) {
            ...
            paths.push(
              <Path
                ...
                key={subidx}
                markerStart={
                  index === 0 && subidx === 0 ? marker.start : undefined
                }
                markerEnd={
