...
export const GotoEmotionFeature: Feature<
  GotoEmotionState,
...
> = {
    ...
    const setShow = useCallback(
      (show: ShowEmotion | null, updateWindowHistory: boolean) => {
        ...
        if (current === undefined) {
          throw new Error(
            'GotoEmotionFeature: setShow called when show is undefined'
          );
        }
  ...
  useResources: (stateVWC, requiredVWC, allStatesVWC) => {
    ...
    const loadingVWC = useMappedValueWithCallbacks(
      ...
      (s) => (s.show !== null && s.show !== undefined ? s.show.emotion : null),
      {
        outputEqualityFn: (a, b) =>
          a === null || b === null ? a === b : a.word === b.word,
      }
    );
    const loadPreventedVWC = useMappedValueWithCallbacks(
      loadingVWC,
      (v) => v === null
    );
    ...
    const premiumEmotionJourneyNR = useNetworkResponse(
      ...
      {
        loadPrevented: premiumClassPreventedVWC,
        minRefreshTimeMS: 0,
        dependsOn: [loadingVWC],
      }
    );
    const imageHandler = useOsehImageStateRequestHandler({});
    const socialProofPicturesVWC = useWritableValueWithCallbacks<
      OsehImageState[]
    >(() => []);
            ...
            (async () => {
              ...
              await apiFetch(
                '/api/1/emotions/started_related_journey',
                {
                  ...
                  body: JSON.stringify({
                    emotion_user_uid:
                      premiumEmotionJourney.result.emotionUserUid,
                  }),
                },
                loginContext
              );
  ...
  component: (state, resources) => (
    <GotoEmotion state={state} resources={resources} />
  ),
};
