...
export type LibraryFilter = Omit<LibraryFilterAPI, '__mapped'> & {
  __mapped: true;
};
...
export const stableSerializeLibraryFilter = (filter: LibraryFilter): string => {
  const sortedInstructors = Array.from(new Set(filter.instructors))
    .sort()
    .join(',');
...
export const convertLibraryFilterToAPI = (
  filter: LibraryFilter
): LibraryFilterAPI => filter as any as LibraryFilterAPI;
