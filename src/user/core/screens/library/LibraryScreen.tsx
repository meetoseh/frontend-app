...
> = {
  ...
  paramMapper: (params) => ({
    ...
    journeyTrigger: convertUsingMapper(
      params.journey_trigger,
      screenConfigurableTriggerMapper
    ),
  ...
  initInstanceResources: (ctx, screen) => {
    ...
    const listRequestVWC =
      createWritableValueWithCallbacks<RequestResult<LibraryListState> | null>(
        null
      );
    ...
    const [simpleListVWC, cleanupSimpleListMapper] =
      createMappedValueWithCallbacks(listVWC, (v) =>
          ...
          if (existingRequest !== null) {
            ...
            if (data.type === 'success') {
              ...
              if (serverNow >= data.data.expiresAtServerMS) {
                ...
                expiresAtPromise = createCancelableTimeout(
                  data.data.expiresAtServerMS - serverNow
                );
            ...
            await Promise.race([
              ...
              ...(expiresAtPromise === undefined
                ? []
                : [expiresAtPromise.promise]),
            ]);
            userChanged.cancel();
            dataChanged.cancel();
              ...
              refreshRef: () =>
                constructCancelablePromise({
                  body: async (state, resolve, reject) => {
                    ...
                    const innerCanceled = createCancelablePromiseFromCallbacks(
                      state.cancelers
                    );
                        ...
                        (v) =>
                          ...
                          (v.state === 'logged-in' &&
                            (v.userAttributes.sub !== user.userAttributes.sub ||
                              getJwtExpiration(v.authTokens.idToken) >
                                refreshAtServerMS))
                      );
                    ...
                    if (
                      newUser.userAttributes.sub !== user.userAttributes.sub
                    ) {
                      innerCanceled.cancel();
                      state.finishing = true;
                      state.done = true;
    ...
    const [instructorListVWC, cleanupInstructorListUnwrapper] =
      unwrapRequestResult(
