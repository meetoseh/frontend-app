...
> = {
  ...
  initInstanceResources: (ctx, screen, refreshScreen) => {
    ...
    const [imageSizeImmediateVWC, cleanupImageSizeImmediate] =
      createMappedValueWithCallbacks(ctx.windowSizeImmediate, (size) => ({
        width: size.width,
        ...
        const req = ctx.resources.offeringHandler.request({
          ...
          refreshRef: () => ({
            promise: Promise.reject(
              new Error('login refresh not supported here')
            ),
            done: () => true,
            cancel: () => {},
          }),
    ...
    const [offeringUnwrappedVWC, cleanupOfferingUnwrapper] =
      unwrapRequestResult(
        offeringVWC,
        (d) => d.data,
        () => null
    ...
    const getOfferings = () =>
      createLoginContextRequest({
        ctx,
        handler: ctx.resources.purchaseOfferingsHandler,
      });
    const offeringsRequestVWC =
      createWritableValueWithCallbacks<RequestResult<PurchasesOfferings> | null>(
        null
      );
    const cleanupOfferingsRequester = createValueWithCallbacksEffect(
      ctx.login.value,
      () => {
        const req = getOfferings();
        setVWC(offeringsRequestVWC, req);
        return () => {
          req.release();
          if (Object.is(offeringsRequestVWC.get(), req)) {
            setVWC(offeringsRequestVWC, null);
          }
        };
      }
    );
    const [offeringsUnwrappedVWC, cleanupOfferingsUnwrapper] =
      unwrapRequestResult(
        offeringsRequestVWC,
        (d) => d.data,
        () => null
    ...
    const cleanupShouldSkipEffect = createValueWithCallbacksEffect(
      offeringVWC,
      (req) => {
        if (req === null) {
    ...
    const cleanupPricesEffect = createValuesWithCallbacksEffect(
      ...
      () => {
        ...
        const offerings = offeringsUnwrappedVWC.get();
        ...
        offer.packages.forEach((p) => {
          const req = ctx.resources.priceHandler.request({
            ref: {
              user,
              offeringIdentifier: offer.identifier,
              offerings,
              platformProductIdentifier: p.platformProductIdentifier,
              platformProductPlanIdentifier: p.platformProductPlanIdentifier,
            },
            refreshRef: () => ({
              promise: Promise.reject(
                new Error('login refresh not supported here')
