...
> = {
  slug: 'series_details',
  paramMapper: (params) =>
    convertUsingMapper(params, seriesDetailsParamsMapper),
  initInstanceResources: (ctx, screen, refreshScreen) => {
    ...
    const journeysRequest =
      createWritableValueWithCallbacks<RequestResult<CourseJourneys> | null>(
        getJourneys()
      );
    ...
    >([]);
    const cleanupJourneyBackgroundHeightsEffect =
      createValueWithCallbacksEffect(journeysUnwrapped, (journeys) => {
        if (journeys === null) {
              ...
              (height) => {
                const getPlaylist = () =>
                  createChainedRequest(
                    getJourneys,
                    ctx.resources.privatePlaylistHandler,
                    {
                      sync: (journeys) => {
                        ...
                        const ref =
                          journeys.journeys[journeyIdx].journey
                            .darkenedBackground;
                        if (ref.jwt === null) {
                          throw new Error(
                            'Expected private playlist, got public playlist'
                          );
                        }
                ...
                const getExportCropped = () =>
                  createChainedRequest(
                    getExport,
                    ctx.resources.imageCropHandler,
                    {
                      sync: (exp) => ({
                        export: exp,
                        cropTo: {
...
const getBackgroundImage = (
  ...
  const cleanupWindowResizeEffect = createValueWithCallbacksEffect(
