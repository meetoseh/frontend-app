...
export type JournalEntryManagerMinimalRef = Pick<
...
const getRefUid = (ref: JournalEntryManagerMinimalRef): string =>
  ref.journalEntryUID;
...
const compareRefs = (
  a: JournalEntryManagerRef,
  b: JournalEntryManagerRef
): number =>
 ...
 */
export const createJournalEntryManager = (
  initial: JournalEntryManagerRef
): JournalEntryManager => {
  const journalEntryUID = initial.journalEntryUID;
  const disposedVWC = createWritableValueWithCallbacks(false);
  const journalEntryJWTVWC = createWritableValueWithCallbacks(
    initial.journalEntryJWT
  );
  ...
  const [journalEntryJWTExpiresAtVWC, cleanupJournalEntryJWTExpiresAt] =
    createMappedValueWithCallbacks(journalEntryJWTVWC, (jwt) =>
  ...
  const dangerousCreateAttachTask: JournalEntryManager['dangerousCreateAttachTask'] =
    (journalChatJWT, clientKey, opts): CancelablePromise<void> => {
      return constructCancelablePromise({
        body: async (state, resolve, reject) => {
          ...
          try {
            ...
            console.warn(
              'JournalEntryManager error in manageWebsocketLoop:',
              e
            );
            setVWC(errorVWC, described);
            setVWC(chatVWC, undefined);
  ...
  const attach: JournalEntryManager['attach'] = (
    journalChatJWT,
    clientKey,
    opts
  ) => {
    ...
    (user, visitor, opts) => {
      return constructCancelablePromise({
        body: async (state, resolve, reject) => {
          ...
          const bonusParams = await (opts?.bonusParams ?? (async () => ({})))(
            clientKey
          );
            ...
            for (let i = 0; ; i++) {
              ...
              if (i > 0) {
                ...
                const timeout = createCancelableTimeout(
                  1000 * Math.pow(2, i) + Math.random() * 500
                );
          ...
          const attacher = dangerousCreateAttachTask(
