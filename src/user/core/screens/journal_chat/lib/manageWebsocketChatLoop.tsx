...
import { HTTP_WEBSOCKET_URL } from '../../../../../shared/lib/apiFetch';
import {
    ...
    body: async (state, resolve, reject) => {
      ...
      while (true) {
        ...
        try {
          ...
          const now = Date.now();
          recentRetriesAt = recentRetriesAt.filter(
            (at) => at > now - 1000 * 300
          );
          ...
          const backoffMs =
            1000 *
            Math.pow(2, recentRetriesAt.length) *
            (Math.random() * 1.5 + 0.5);
  ...
  return constructCancelablePromise({
    body: async (state, resolve, reject) => {
      ...
      }
      {
        ...
        initialState.integrity = await computeJournalChatStateDataIntegrity(
          initialState
        );
        if (state.finishing) {
          canceled.cancel();
          ...
          try {
            {
              ...
              if (!authResponse.success) {
                state.finishing = true;
                console.warn(
                  'authorization rejected on chat websocket:',
                  authResponse
                );
                state.done = true;
            ...
            while (true) {
              ...
              if (!message.success) {
                console.log(
                  'unexpected top-level error in chat; treating as retryable: ',
                  message
                );
                state.finishing = true;
                state.done = true;
              ...
              for (const event of events) {
                if (
                  event.type === 'thinking-bar' ||
                  event.type === 'thinking-spinner'
                ) {
                  chatState.transient = event;
                } else if (event.type === 'error') {
                  console.warn(
                    'server experienced error producing system response',
                    event
                  );
                  state.finishing = true;
                  state.done = true;
              ...
              const expectedIntegrity =
                await computeJournalChatStateDataIntegrity(chatState, false);
