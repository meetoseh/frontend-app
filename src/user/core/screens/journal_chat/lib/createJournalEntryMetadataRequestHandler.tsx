...
export type JournalEntryMetadataMinimalRef = Pick<
...
const getDataFromRef = createGetDataFromRefUsingSignal({
  inner: async (
    ...
    const decryptedPayloadStr = await fernet.decrypt(
...
const compareRefs = (
  a: JournalEntryMetadataRef,
  b: JournalEntryMetadataRef
): number =>
  Math.min(
    getJwtExpiration(b.jwt),
    getJwtExpiration(b.user.authTokens.idToken)
  ) -
  Math.min(
    getJwtExpiration(a.jwt),
