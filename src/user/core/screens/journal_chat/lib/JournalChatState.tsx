...
export const computeJournalChatStateDataIntegrity = async (
  ...
  const stableSerializedAsUint8Array = new Uint8Array(
    Buffer.from(stableSerializedAsUtf8String, 'utf8')
  );
  const digestedAsArrayBuffer = await Crypto.digest(
    Crypto.CryptoDigestAlgorithm.SHA256,
    stableSerializedAsUint8Array
  );
  const digestedAsUint8Array = new Buffer(digestedAsArrayBuffer);
  const digestedAsHexString = digestedAsUint8Array.toString('hex');
...
const dumpJournalEntryItemDataForIntegrity = (
  data: JournalEntryItemData
): string => {
  let raw = '{"data": ';
...
const dumpJournalEntryItemTextualPartJourneyForIntegrity = (
  ...
  let raw = '{"details": ';
  raw += dumpJournalEntryItemTextualPartJourneyDetailsForIntegrity(
    data.details
  );
...
const dumpOsehTranscriptPhraseAPIForIntegrity = (
  data: OsehTranscriptPhraseAPI
): string => {
  let raw = '{"ends_at": ';
...
const dumpJournalEntryItemTextualPartForIntegrity = (
  data: JournalEntryItemTextualPart
): string => {
...
const dumpJournalEntryItemDataDataUIForIntegrity = (
...
const dumpJournalEntryItemUIFlowForIntegrity = (
  data: JournalEntryItemUIFlow
): string => {
  let raw = '{"slug": ';
...
const dumpJournalEntryItemDataDataForIntegrity = (
  data: JournalEntryItemDataData
): string => {
