    ...
    while (true) {
      ...
      switch (previous.type) {
        ...
        case 'incremental-refresh-preparing-references':
          await transitionFromIncrementalRefreshPreparingReferences(
            stateVWC,
            messageVWC
          );
          break;
        case 'incremental-refresh-preparing-client-key':
          await transitionFromIncrementalRefreshPreparingClientKey(
            stateVWC,
            messageVWC
          );
          break;
        case 'incremental-refresh-authorizing':
          await transitionFromIncrementalRefreshAuthorizing(
            stateVWC,
            messageVWC
          );
          break;
...
const transitionFromInitializing = async (
  ...
  if (messageVWC.get() !== null) {
    ...
    throw new Error(
      `unexpected msg in ${current.type}: ${JSON.stringify(msg)}`
    );
  }
    ...
    setVWC(stateVWC, {
      ...
      error:
        e instanceof DisplayableError
          ? e
          : new DisplayableError('client', ACTION, `${e}`),
    });
    return;
  }
  setVWC(stateVWC, {
...
const transitionFromAuthorizing = async (
      ...
      error:
        authorizeResult.type === 'error'
          ...
          : new DisplayableError(
              'client',
              ACTION,
              `authorize state: ${authorizeResult.type}`
            ),
    });
    return;
  }
  ...
  setVWC(stateVWC, {
    ...
    websocket: createSmartWebsocketConnectForJournalChat(
      authorizeResult.value.journal_chat_jwt
    ),
    value: {
      displayable:
        createWritableValueWithCallbacks<JournalChatState>(chatState),
    },
  });
};
...
const applyIncomingChat = async (
  ...
  try {
    ...
    segmentDataString = await clientKey.key.decrypt(
      data.encrypted_segment_data,
      nowServer
    );
  } catch (e) {
...
const applyIncomingMessage = async (
  ...
  try {
    msgAsString =
      typeof msg === 'string' ? msg : new TextDecoder('utf-8').decode(msg);
  ...
  for (let i = 0; i < parsed.data.events.length; i++) {
    ...
    switch (eventData.type) {
      case 'thinking-bar':
        result = applyIncomingThinkingBarMessage(
          toMutate,
          msgAsString,
          eventData
        );
      ...
      case 'thinking-spinner':
        result = applyIncomingThinkingSpinnerMessage(
          toMutate,
          msgAsString,
          eventData
        );
...
const transitionFromStreaming = async (
  ...
  while (true) {
    ...
    const incomingVWC =
      wsState.type === 'open' || wsState.type === 'closed'
        ? wsState.value.incoming
        : null;
    if (incomingVWC !== null) {
  ...
  if (msg.type === 'incremental-refresh') {
    ...
    setVWC(stateVWC, {
      ...
      value: {
        ...
        displayable:
          createWritableValueWithCallbacks<JournalChatState>(displayable),
      },
    });
    return;
  }
...
const transitionFromIncrementalRefresh = async (
  stateVWC: WritableValueWithCallbacks<State>,
  _messageVWC: WritableValueWithCallbacks<Message | null>
) => {
  const current = stateVWC.get();
    ...
    if (incomingVWC !== null) {
      ...
      if (incoming !== undefined) {
        ...
        try {
          ...
          if (
            !isAnticipatedStateStillPlausible(
              current.value.anticipated,
              currentReceived
            )
          ) {
            console.log(
              'switching to streaming as state is no longer plausible'
            );
            console.log(
              '  anticipated:',
              deepClonePrimitives(current.value.anticipated)
            );
