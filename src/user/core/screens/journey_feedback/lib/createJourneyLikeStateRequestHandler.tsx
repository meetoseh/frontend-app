...
const getRefUid = (ref: { journey: { uid: string } }): string =>
  ref.journey.uid;
const getDataFromRef = (
  loginContextRaw: LoginContextValue
): ((
  ref: ExpirableJourneyRef
) => CancelablePromise<Result<JourneyLikeState>>) => {
  return (ref) => {
    ...
    return constructCancelablePromise({
      body: async (state, resolve, reject) => {
        ...
        try {
          ...
          const likedAtRaw = data.items[0].liked_at as
            | number
            | null
            | undefined;
          const likedAt =
            likedAtRaw === null || likedAtRaw === undefined
              ? null
              : new Date(likedAtRaw * 1000);
          state.finishing = true;
