...
const getRefUid = (ref: JournalEntryListMinimalRequest): string =>
...
>({
  inner: async (ref, signal) => {
    ...
    const userTokenExpiresAtServer = getJwtExpiration(
      ref.user.authTokens.idToken
    );
...
const compareRefs = (
  a: JournalEntryListRequest,
  b: JournalEntryListRequest
): number =>
