...
> = {
  ...
  initInstanceResources: (ctx, screen, refreshScreen) => {
    ...
    const journalEntryMetadataRequestVWC =
      createWritableValueWithCallbacks<RequestResult<JournalEntryMetadata> | null>(
        null
      );
    const journalEntryMetadataErroredVWC =
      createWritableValueWithCallbacks<boolean>(false);
    ...
    const cleanupJournalEntryMetadataRequester = (() => {
      ...
      async function makeRequest() {
        ...
        const jwt =
          journalEntryJWTVWC.get() ?? screen.parameters.journalEntry.jwt;
        const request = getJournalEntryMetadata(
          user,
    ...
    const [
      journalEntryMetadataUnwrappedVWC,
    ...
    >(null);
    const cleanupJournalEntryMetadataMergerVWC =
      createValuesWithCallbacksEffect(
    ...
    const cleanupReady = createValuesWithCallbacksEffect(
      [chatVWC, journalEntryMetadataVWC],
      () => {
        const chat = chatVWC.get();
        setVWC(
          readyVWC,
          chat !== null || journalEntryMetadataVWC.get() === undefined
        );
        return undefined;
      }
    );
