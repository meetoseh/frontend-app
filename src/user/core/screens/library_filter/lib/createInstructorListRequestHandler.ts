...
const getRefUid = (ref: InstructorListMinimalRequest): string =>
...
>({
  inner: async (ref, signal) => {
    ...
    while (true) {
      ...
      const data: {
        items: SearchPublicInstructorAPI[];
        next_page_sort?: CrudFetcherSort | null;
      } = await response.json();
      result.push(
        ...data.items.map((v) =>
          convertUsingMapper(v, searchPublicInstructorMapper)
        )
      );
      ...
      let haveMore = false;
      for (const [_searchKey, searchValue] of Object.entries(
        data.next_page_sort
      )) {
...
const compareRefs = (
  a: InstructorListRequest,
  b: InstructorListRequest
): number =>
