...
export type PromptMergeSuccess = {
  type: "success";
  /**
   * The merge token we received
   */
  mergeToken: string;
};

export type PromptMergeCancel = {
  type: "cancel";
};

export type PromptMergeDismiss = {
  type: "dismiss";
};

export type PromptMergeUnknown = {
  type: "unknown";
  rawType: string;
};

export type PromptMergeError = {
  type: "error";
  error: string;
};
 ...
 */
export type WritableMergeMessagePipe = {
  /**
   ...
   */
  send: (message: PromptMergeResult) => CancelablePromise<void>;
  /**
 ...
 */
export const createWritePipe = async (): Promise<WritableMergeMessagePipe> => {
  const item = readPipe();
  if (item !== null) {
    console.log(
      `MergeMessagePipe createWritePipe found existing pipe: ${JSON.stringify(
        item
      )}, destroying it`
    );
 ...
 */
export type ReadableMergeMessagePipe = {
  /**
   ...
   * died.
   */
  read: () => CancelablePromise<PromptMergeResult>;
...
export const createReadPipeIfAvailable =
  async (): Promise<ReadableMergeMessagePipe | null> => {
    ...
    return {
      read: (): CancelablePromise<PromptMergeResult> => {
        ...
        const promise = (async (): Promise<PromptMergeResult> => {
          if (!active) {
            throw new Error("canceled");
          }
