...
> = {
  ...
  initInstanceResources: (ctx, screen, refreshScreen) => {
    ...
    const summaryVWC = createWritableValueWithCallbacks<
      | { entryCounter: number; data: JournalEntryItemDataDataSummaryV1 }
      | null
      | undefined
    >(null);
          ...
          async function retryUntilHaveSummary() {
            ...
            while (true) {
              ...
              for (let i = chat.data.length - 1; i >= 0; i--) {
                ...
                if (
                  entryItem.type === 'summary' &&
                  entryItem.data.type === 'summary'
                ) {
                  summary = {
                    entryCounter: i + 1,
                    data: Object.assign({}, entryItem.data),
                  };
                }
              }
              ...
              if (user.state !== 'logged-in') {
                console.warn(
                  'failed to retry summary generation: user not logged in'
                );
                setVWC(summaryVWC, undefined);
                await Promise.race([
                  taskChanged.promise,
                ...
                endpoint:
                  screen.parameters.missingSummary.endpoint.length === 0
                    ...
                    : screen.parameters.missingSummary.endpoint[
                        Math.min(
                          screen.parameters.missingSummary.endpoint.length - 1,
                          failures
                        )
                      ],
                unsafeToRetry: false,
