...
import {
  ScreenQueueState,
  UseScreenQueueStateState,
} from './useScreenQueueState';
...
export const useScreenQueue = ({
  ...
  useEffect(() => {
    ...
    function initState(
      id: number,
      state: UseScreenQueueStateState,
      isStackPopped?: boolean
    ) {
      if (!isStackPopped) {
    ...
    async function initLoading(
      ...
      if (!effectMounted || id !== counter) {
        throw new Error(
          'unreachable (dangerous cancel() call in tight section)'
        );
      }
    ...
    async function initError(
      id: number,
      state: UseScreenQueueStateState & { type: 'error' }
    ) {
      if (!effectMounted || id !== counter) {
        return;
      }
    ...
    async function initPeeked(
      id: number,
      state: UseScreenQueueStateState & { type: 'success' }
    ) {
      if (!effectMounted || id !== counter) {
        return;
      }
      ...
      while (true) {
        ...
        if (activeScreen === undefined) {
          ...
          if (skipsInARow > 10) {
            logging.error(
              `${effectUid} | ${loopUid} - too many skips in a row, going to error`
            );
            setVWC(valueVWC, {
              type: 'error',
          ...
          logging.info(
            `${effectUid} | ${loopUid} - processed skip, delegating to reinit`
          );
        ...
        const repeekRequested = createCancelablePromiseFromCallbacks(
          repeekRequestedCallbacks
        );
        ...
        const prefetchResources = state.result.prefetch.map(
          (prefetchInstance, idx) => {
            ...
            return screen.initInstanceResources(
              screenContext,
              prefetchInstanceMapped,
              () =>
                delayCancelableUntilResolved(
                  (u: RefreshResult) => ({
          ...
          if (repeekRequested.done() && active.get()) {
            ...
            screenQueueState.peek().promise.finally(() => {
              ...
              if (effectMounted && reinitId === counter) {
                logging.info(
                  `${effectUid} | ${loopUid} - reiniting after repeek`
                );
                cancel();
                reinit(reinitId);
              }
        ...
        let startPopRequested = createCancelablePromiseFromCallbacks(
        ...
        const component = activeScreen.component({
          ...
          startPop: (trigger, endpoint, onError) => {
            ...
            finishPopRequested =
              createCancelablePromiseFromCallbacks(finishPopCallbacks);
        ...
        while (true) {
          ...
          if (startPopRequested.done()) {
            ...
            logging.info(
              `${effectUid} | ${loopUid} - preparing to pop with trigger ${JSON.stringify(
                ...
                undefined,
                2
              )} via endpoint ${endpoint}; error handler set? ${
                onError !== undefined
              }`
            );
            setVWC(valueVWC, { type: 'preparing-pop', component });
            const popCancelable = screenQueueState.pop(
              state,
              popTrigger,
              endpoint,
              onError
            ...
            if (
              active.get() &&
              (popResult.type !== 'success' ||
                popResult.data.type !== 'success')
            ) {
                  ...
                  logging.warn(
                    `${effectUid} | ${loopUid} - pop failed with error handler and screen changed, going to reinit; ` +
                      `old screen state: ${JSON.stringify(
                        ...
                        undefined,
                        2
                      )}; new screen state: ${JSON.stringify(
                        screenState,
                        undefined,
                        2
                      )}`
              ...
              return screen.initInstanceResources(
                ...
                () => ({
                  promise: Promise.reject(
                    new Error('refresh not supported in this spot')
                  ),
                  cancel: () => {},
                  done: () => true,
                })
          ...
          if (!repeekRequested.done()) {
            ...
            if (
              active.get() &&
              (newResult.type !== 'success' ||
                newResult.data.type !== 'success')
            ) {
              logging.warn(
                `${effectUid} | ${loopUid} - peek failed, delegating to reinit`
              );
              cancel();
              reinit(++counter);
              // PURPOSEFUL FALL THROUGH
