...
export const useFeaturesState = (
  maxSimultaneousLoadedResources = 3
): FeaturesState => {
  ...
  const requiredArr = features.map((s, idx) =>
    s.isRequired(states[idx] as any, allStates)
  );
  ...
  const feature = useMemo<ReactElement | null>(() => {
    ...
    return (firstRequired.feature.component as any).call(
      ...
      (worldState: any, pending: Promise<void>) => {
        ...
        if (
          features[newFirstRequiredIdx].identifier ===
          firstRequired?.feature.identifier
        ) {
          return;
        }
