    ...
    result.promise
      .then((earlierItems) => {
        if (earlierItems !== null && earlierItems.length > 0) {
          this.items = [
            ...earlierItems,
            ...(this.items?.slice(0, -earlierItems.length) ?? []),
          ];
    ...
    result.promise
      .then((laterItems) => {
        if (laterItems !== null && laterItems.length > 0) {
          this.items = [
            ...(this.items?.slice(laterItems.length) ?? []),
            ...laterItems,
          ];
...
export class ProceduralInfiniteListing<T extends object> {
  ...
  async reset(): Promise<void> {
    if (this.fakedNetworkDelay > 0) {
      ...
      await new Promise((resolve) =>
        setTimeout(resolve, this.fakedNetworkDelay)
      );
    }
    this.items = [];
    for (
  ...
  onFirstVisible(): void {
    (async () => {
      ...
      this.items = [
        ...newItems,
        ...this.items.slice(0, this.items.length - numItemsEarlier),
      ];
      this.locked = false;
 ...
 */
export class PrefixedNetworkedInfiniteListing<
  DataT extends object,
  PrefixT extends object
> {
  ...
  constructor(delegate: InfiniteListing<DataT>, prefix: PrefixT[]) {
    ...
    this.delegate.onShiftedEarlier.add(
      this.onDelegateShiftedEarlier.bind(this)
    );
...
class CachedServerList<T extends object> {
  ...
  tryPopAndUnshift(): CancelablePromise<T[] | null> {
    ...
    const promise = (async (): Promise<T[] | null> => {
      ...
      try {
        await Promise.race([
          canceledPromise.promise,
          fetchBeforePromise.promise,
        ]);
      } catch (e) {
  ...
  private loadWithSort(
    ...
    const promise = (async (): Promise<ServerListResponse<T>> => {
      ...
      const haveMore =
        ...
        data.next_page_sort.some(
          (s) => s.after !== null && s.after !== undefined
        );
      active = false;
      return {
        items,
