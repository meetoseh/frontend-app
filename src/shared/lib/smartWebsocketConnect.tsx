import { HTTP_WEBSOCKET_URL } from './apiFetch';
import {
...
async function transitionFromWaiting<T extends {} | null>(
...

  {
    ...
    if (now < current.nextAttemptAt) {
      const timeoutPromise = createCancelableTimeout(
...
async function transitionFromInFlight<T extends {} | null>(
  ...
  const outgoing = createWritableValueWithCallbacks<
    (string | ArrayBuffer) | null
  >(null);
  const wsState = createWritableValueWithCallbacks<
  ...
  const wsClosed = waitForValueWithCallbacksConditionCancelable(
    wsState,
    (s) => s === 'closed'
  );
  ...
  try {
    ...
    const err =
      e instanceof DisplayableError
        ? e
        : new DisplayableError('client', 'greet websocket', `${e}`);
...
async function transitionFromOpen<T extends {} | null>(
  ...
  while (true) {
    ...
    if (msgReceived.done()) {
      ...
      if (msg.type === 'retry') {
        const nextAttempt = current.retryer(
          current.attempt,
          msg.retryAfterMS ?? null
        );
        if (nextAttempt.delay !== undefined) {
          setVWC(stateVWC, {
            type: 'waiting',
...
export function createSmartWebsocketConnect<T extends {} | null>({
  ...
  const messageVWC =
    createWritableValueWithCallbacks<SmartWebsocketConnectMessage | null>(null);
