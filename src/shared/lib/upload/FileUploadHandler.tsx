...
const uploadPart = async (
  ...
  formData.append(
    "file",
    file.slice(startByte, endByte, "application/octet-stream"),
    file.name
  );
  ...
  for (const part of parts) {
    ...
    if (partNumber >= partRange.startNumber && partNumber < partEndNumberExcl) {
      return {
        ...
        startByte:
          partRange.startByte +
          (partNumber - partRange.startNumber) * partRange.partSize,
        endByte:
          partRange.startByte +
          (partNumber - partRange.startNumber + 1) * partRange.partSize,
      };
    }
  }
...
export const FileUploadHandler = ({
  ...
  useEffect(() => {
    ...
    async function uploadFile() {
      const lastPart = convertToRange(
        uploadInfo.parts[uploadInfo.parts.length - 1]
      );
      const endPartNumber = lastPart.startNumber + lastPart.numberOfParts;
      const bytesTotal =
        lastPart.startByte + lastPart.numberOfParts * lastPart.partSize;
      const parallel = 5;
      ...
      while (nextPartNumber < endPartNumber || numUploading > 0) {
        ...
        while (nextPartNumber < endPartNumber && numUploading < parallel) {
          numUploading++;
          uploading[nextPartNumber.toString()] =
            /* eslint-disable-line no-loop-func*/ (async (
              file,
              part,
              uploadInfo
            ) => {
              ...
              numUploading--;
              setUploadedBytes(
                (uploadedBytes) => uploadedBytes + part.endByte - part.startByte
              );
            })(
              file,
              getSinglePart(uploadInfo.parts, nextPartNumber),
              uploadInfo
            );
          nextPartNumber++;
        }
