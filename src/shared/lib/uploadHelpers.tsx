...
const autoCombiningInsert = (() => {
  const mergeRight = (
    ...
    while (arr.list.length > index + 1) {
      if (
        arr.list[index].number + arr.list[index].numberOfParts ===
          arr.list[index + 1].number &&
  ...
  const mergeLeft = (
    ...
    while (index > 0) {
      if (
        arr.list[index].number ===
          arr.list[index - 1].number + arr.list[index - 1].numberOfParts &&
        arr.list[index].partSize === arr.list[index - 1].partSize
      ) {
...
export type UploadPartStateAcquiringData = UploadPart & {
  ...
  task: CancelablePromise<ArrayBuffer>;
};
...
export type UploadProgress = {
  ...
  inprogress: {
    ...
    acquiringData: ValueWithCallbacks<
...
export type UploadArgs = {
   ...
   * the part.
   */
  getData: (part: UploadPart) => CancelablePromise<ArrayBuffer>;
   ...
   */
  tryUpload: (
    part: Omit<UploadPartStateUploading, 'task'>
  ) => CancelablePromise<TryUploadResult>;
};
...
export const upload = ({
  ...
  const finishedVWC = createWritableValueWithCallbacks<
    SortedList<UploadPartRange, number>
  >(new SortedList((v) => v.number));
  const erroredVWC = createWritableValueWithCallbacks<
    SortedList<UploadPartRange, number>
  >(new SortedList((v) => v.number));
  ...
  const remainingVWC = createWritableValueWithCallbacks<
    SortedList<UploadPartRange, number>
  >(
    (() => {
      ...
      for (const part of parts) {
        if (
          ...
          result.list[result.list.length - 1].number < part.number
        ) {
  ...
  const stateVWC = createWritableValueWithCallbacks<
  ...
  function cancelEverythingAndSetError() {
    console.trace('cancelEverythingAndSetError!');
  ...
  async function sweepUploading(): Promise<'continue' | 'errored'> {
    ...
    while (i < uploading.list.length) {
      ...
      try {
        ...
        console.log('sweepUploading promise rejected:', e);
      ...
      if (
        result.type === 'retryable-error' &&
        part.retryCounter < retries.max
      ) {
        const retryTimeMS = Math.max(
          retries.backoff(part.retryCounter),
          result.minRetryTimeMS
        );
        ...
        retriedVWC.set({
          ...
          highestCounter: Math.max(
            oldRetried.highestCounter,
            retryPart.retryCounter
          ),
        });
  ...
  async function sweepAcquiringData(): Promise<
    'continue' | 'need-upload-spot' | 'errored'
  > {
    let i = 0;
  ...
  async function sweepWaitingToRetry(): Promise<
    'continue' | 'need-upload-spot'
  > {
    while (true) {
      ...
      if (part === undefined) {
        throw new Error(
          'peeked was not undefined, but shift returned undefined'
        );
      }
  ...
  async function sweepRemaining(): Promise<
    'all-queued' | 'need-acquire-data-spot'
  > {
    while (true) {
      ...
      while (true) {
        ...
        // sweep remaining
        {
          ...
          if (
            ...
            acquiringData.list[0].task.done()
          ) {
            continue;
          }
          ...
          if (
            ...
            if (result !== 'all-queued') {
              cancelEverythingAndSetError();
              throw new Error(
                'remaining parts were not all queued despite nothing happening'
              );
            }
...
/**
 * NATIVE ONLY: the standard get data function for an API fetch where the
 * data is on the local file system
 */
export const uploadStandardURIGetData =
  (uri: string): ((part: UploadPart) => CancelablePromise<ArrayBuffer>) =>
  (part) =>
    constructCancelablePromise<ArrayBuffer>({
      body: async (state, resolve, reject) => {
        ...
        try {
          dataBase64 = await FileSystem.readAsStringAsync(uri, {
            position: part.startByte,
            length: part.endByte - part.startByte,
            encoding: 'base64',
          });
        } catch (e) {
        ...
        let asBuffer: Buffer;
        try {
          asBuffer = Buffer.from(dataBase64, 'base64');
        } catch (e) {
          state.finishing = true;
          state.done = true;
          reject(new Error('failed to convert base64 to buffer'));
          return;
        }
    ...
    constructCancelablePromise({
      body: async (state, resolve, reject) => {
        await adaptCallbacksToAbortSignal(state.cancelers, async (signal) => {
          // react native does not support FormData, so we have to format the data ourself
          const boundary =
          ...
          try {
            ...
            console.warn('apiFetch failed with:', e);
            if (state.finishing) {
              state.done = true;
              reject(new Error('canceled'));
              return;
          ...
          if (retryAfterRaw !== null) {
            ...
            if (!isNaN(retryAfter) && retryAfter > 0 && retryAfter <= 180) {
              ...
              resolve({
                type: 'retryable-error',
                minRetryTimeMS: retryAfter * 1000,
              });
              return;
            }
          }
