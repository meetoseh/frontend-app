 ...
 */
export type TransitionPropAsInitialized<
  T extends string,
  C extends { type: T; ms: number }
> = {
 ...
 * ```
 */
export type TransitionPropAsOwner<
  T extends string,
  C extends { type: T; ms: number }
> = {
  animation: WritableValueWithCallbacks<C>;
 ...
 */
export const useInitializedTransitionProp = <
  T extends string,
  C extends { type: T; ms: number }
>(
  ...
  const initialized = useRef<TransitionProp<T, C>>(null) as MutableRefObject<
    TransitionProp<T, C>
  >;
 ...
 */
export const playEntranceTransition = <
  T extends string,
  C extends { type: T; ms: number }
>(
    ...
    body: async (state, resolve, reject) => {
      ...
      if (!transition.ready.get()) {
        while (true) {
          ...
          const readyChanged = createCancelablePromiseFromCallbacks(
            transition.ready.callbacks
          );
 ...
 */
export const useEntranceTransition = <
  T extends string,
  C extends { type: T; ms: number }
>(
  transition: TransitionPropAsOwner<T, C>,
  ...
  const unblocked = useWritableValueWithCallbacks(
    () => blocked === undefined || !blocked.get()
  );
  ...
  useValueWithCallbacksEffect(
    ...
    useCallback(
      (unblocked) => {
        ...
        let active = true;
        const entrance = playEntranceTransition(transition, {
          returnWhen: 'animation-started',
        });
 ...
 */
export const playExitTransition = <
  T extends string,
  C extends { type: T; ms: number }
>(
  transition: TransitionPropAsOwner<T, C>,
  ...
  return constructCancelablePromise({
    body: async (state, resolve, reject) => {
      ...
      while (true) {
        ...
        await Promise.race([
          animatingChanged.promise,
          readyChanged.promise,
          canceled.promise,
        ]);
        animatingChanged.cancel();
        readyChanged.cancel();
