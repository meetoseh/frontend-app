...
export const colorToCSS = (
  ...
  return `rgba(${colorFloatToByte(color[0])}, ${colorFloatToByte(
    color[1]
  )}, ${colorFloatToByte(color[2])}, ${color[3]})`;
};
...
export const fractionalColorToCss = (
  ...
  return `rgb(${colorFloatToByte(color[0])}, ${colorFloatToByte(
    color[1]
...
export const makeLinePath = (data: number[]): string => {
  ...
  for (let i = 1; i < numPoints; i++) {
    parts.push(
      `L${makeSVGNumber(data[i * 2])} ${makeSVGNumber(data[i * 2 + 1])}`
    );
  }
  return parts.join("");
};
