...
type EngineState = {
  ...
  running: {
    ...
    animation: DynamicAnimationEngineItemArg;
  }[];
...
export const useDynamicAnimationEngine = (): DynamicAnimationEngine => {
  const engineVWC = useWritableValueWithCallbacks<EngineState | null>(
    () => null
  );
  const playing = useMappedValueWithCallbacks(
    engineVWC,
    (engine) => engine !== null
  );
        ...
        engineVWC,
        {
          ...
          animationsRelativeToStart: (() => {
            ...
            for (const anim of animations) {
              if (
                anim.delayUntil === undefined ||
                anim.delayUntil.type !== 'relativeToStart'
              ) {
                continue;
              }
            ...
            while (!next.done) {
              next.value.sort(
                (a, b) => a.delayUntil.after - b.delayUntil.after
              );
              next = iter.next();
            }
          ...
          animationsRelativeToEnd: (() => {
            ...
            for (const anim of animations) {
              if (
                anim.delayUntil === undefined ||
                anim.delayUntil.type !== 'relativeToEnd'
              ) {
                continue;
              }
  ...
  useValueWithCallbacksEffect(engineVWC, (engineRaw) => {
    ...
    function processTick(now: DOMHighResTimeStamp): boolean {
      const delta =
        engine.lastFrameAt === undefined ? 0 : now - engine.lastFrameAt;
      ...
      for (
        let wrapperIdx = 0;
        wrapperIdx < engine.running.length;
        wrapperIdx++
      ) {
        ...
        const linearProgress = done
          ? 1
          : Math.min(relativeTime / wrapper.animation.duration, 1);
        const progress = done
          ? 1
        ...
        if (done) {
          ...
          const toStart = engine.animationsRelativeToEnd.get(
            wrapper.animation.id
          );
      ...
      return (
        engine.running.length !== 0 || engine.animationsWithDelay.length !== 0
      );
    }
    ...
    function startAnimationAfterDelay(
      ...
      const insertIndex = engine.animationsWithDelay.findIndex(
        (a) => a.ms > absoluteStartTime
      );
      if (insertIndex === -1) {
        engine.animationsWithDelay.push({
