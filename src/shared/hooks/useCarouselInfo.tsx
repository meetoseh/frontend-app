...
export const useCarouselInfo = ({
  ...
  const settingsVWC = useVariableStrategyPropsAsValueWithCallbacks(
    settingsVariableStrategy
  );
  ...
  const panningVWC = useVariableStrategyPropsAsValueWithCallbacks(
    panningVariableStrategy
  );
  ...
  const inClickCooldownVWC = useWritableValueWithCallbacks<boolean>(
    () => false
  );
  ...
  useEffect(() => {
    ...
    function onFirstFrame(
      oldPerfNow: number,
      newFrameTime: DOMHighResTimeStamp
    ) {
      handleFrame(performance.now() - oldPerfNow, newFrameTime);
    }

    function onFrame(
      oldFrameTime: DOMHighResTimeStamp,
      newFrameTime: DOMHighResTimeStamp
    ) {
  ...
  return [
    ...
    useCallback(
      (offset: number) => {
        if (!panningVWC.get()) {
          throw new Error(
            "cannot use the panToOffset function when not panning"
          );
        }
