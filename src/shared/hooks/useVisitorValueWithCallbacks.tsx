...
type VisitorAFSMEventUserChanged = {
  type: 'userChanged';
  user: LoginContextValueLoggedIn | null;
};
...
type VisitorAFSMHandlerResult = {
  ...
  onEvent: {
    [K in VisitorAFSMEventType]: (
      event: VisitorAFSMEvent & { type: K }
    ) => void;
  };
};
...
const handlers: VisitorAFSMHandlers = {
  loadingFromStore: (state) => {
    const newState = createWritableValueWithCallbacks<
      VisitorAFSMState | null | undefined
    >(null);
    let active = true;
...
export const useVisitorValueWithCallbacks = (
  ...
  useEffect(() => {
    ...
    function onChanged() {
      stateMachine.get().result.onEvent.utmChanged({
        type: 'utmChanged',
  ...
  const setVisitor = useCallback(
    (uid: string) => {
      stateMachine
        .get()
        .result.onEvent.visitorForced({ type: 'visitorForced', uid });
    },
    [stateMachine]
  );
  ...
  const mappedStateMachineVWC = useMappedValueWithCallbacks(
    stateMachine,
    (v): VisitorValue => {
