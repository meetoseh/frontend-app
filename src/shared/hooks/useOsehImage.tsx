...
const arePropsEqual = (a: OsehImageProps, b: OsehImageProps): boolean => {
  return (
    a.uid === b.uid &&
    a.jwt === b.jwt &&
...
/**
 * Crops the image available at the given local url to the given size from
 * the center. Prefer using the cropImage wrapper.
 *
 * This has to deal with the fact that the ImageManipulator library doesn't
 * expose a raw function to just get the image info without copying the
 * file, and that we can't control where the image is saved, so it has to
 * do way more disk work than it should.
 */
...
const downloadItem = async (
  ...
  if (opts !== undefined && opts.cropTo !== undefined) {
    const cropped = await cropImage(
      targetFile,
      opts.cropTo,
      `${opts.cropTo.width}-${opts.cropTo.height}-${uidWithExt}`
    );
    return {
      remoteUrl: item.url,
      ...
      playlists.forEach((playlist, uid) => {
        ...
        const bestItem = selectBestItemUsingPixelRatio({
          ...
          preferredPixelRatio: Math.min(
            want.width / props.displayWidth,
            want.height / props.displayHeight
          ),
        });
        if (
          old === null ||
  ...
  useEffect(() => {
    ...
    async function fetchDownloadedItem(
      ...
      if (old !== null && old.remoteUrl === item.item.url) {
        ...
        const recropped = await cropImage(
          old.originalLocalUrl,
          item.cropTo,
          `${item.cropTo.width}-${item.cropTo.height}-${item.item.url.split('/').pop()}`
        );
        return {
          remoteUrl: item.item.url,
          localUrl: recropped,
...
export const useOsehImageStatesRef = ({
  ...
  useEffect(() => {
    ...
    async function handleUsingPlaylist(
      ...
      const want = {
        width: PixelRatio.getPixelSizeForLayoutSize(initialProps.displayWidth),
        height: PixelRatio.getPixelSizeForLayoutSize(initialProps.displayHeight),
      };
      const bestImage = selectBestItemUsingPixelRatio({
        ...
        preferredPixelRatio: Math.min(
          want.width / initialProps.displayWidth,
          want.height / initialProps.displayHeight
        ),
      });
      ...
      if (uncroppedCached) {
        ...
        const croppedLocalUrl = bestImage.cropTo
          ? await cropImage(
              uncroppedLocalUrl,
              bestImage.cropTo,
              `${bestImage.cropTo.width}-${bestImage.cropTo.height}-${bestImage.item.url
                .split('/')
                .pop()}`
            )
          : uncroppedLocalUrl;
...
/**
 * Removes the image with the given uid from the handling list of the given
 * image states ref, if it's present, otherwise does nothing. This calls
 * onHandlingChanged if appropriate.
 */
export const removeImageIfPresent = (imageStates: OsehImageStatesRef, uid: string) => {
  const oldProps = imageStates.handling.current.get(uid);
  if (oldProps === undefined) {
    return;
  }

  imageStates.handling.current.delete(uid);
  imageStates.onHandlingChanged.current.call({
    old: oldProps,
    current: null,
    uid,
  });
...
/**
 * Ensures that the given image states is handling the image with the given uid with
 * the given props, calling onHandlingChanged if appropriate.
 *
 * @param imageStates The image states ref to ensure the image is being handled by
 * @param uid The uid of the image to ensure is being handled
 * @param props The props to ensure the image is being handled with
 */
export const ensureHandlingImage = (
  imageStates: OsehImageStatesRef,
  uid: string,
  props: OsehImageProps | null
) => {
  if (props === null) {
    removeImageIfPresent(imageStates, uid);
    return;
  }

  const oldProps = imageStates.handling.current.get(uid) ?? null;
  if (oldProps === props || (oldProps !== null && arePropsEqual(oldProps, props))) {
    return;
  }

  imageStates.handling.current.set(uid, props);
  imageStates.onHandlingChanged.current.call({
    old: oldProps,
    current: props,
    uid,
