...
export const useMinHeights = <K extends string>(
  ...
  if (keysRef.current === null) {
    ...
    if (oldKeysSet.size !== newKeysSet.size) {
      throw new Error(
        'useMinHeights: keys changed between renders. This is not allowed.'
      );
    }
  ...
  const devicePixelRatioVWC = useMappedValueWithCallbacks(
    windowSizeVWC,
    (s) => s.fontScale
  );
  const minRelevantPixelDifferenceVWC = useMappedValueWithCallbacks(
    devicePixelRatioVWC,
    (devicePixelRatio) => 1 / devicePixelRatio + 1e-6
  );
  ...
  if (minHeights.current === null) {
    ...
    for (const key in baseInitial) {
      newValue[key] = createWritableValueWithCallbacks(
        baseInitial[key].minHeight
      );
    }
  ...
  keysRef.current.forEach((key) => {
    ...
    useValuesWithCallbacksEffect(
      [requested[key], refs[key].size, minRelevantPixelDifferenceVWC],
      () => {
        ...
        function onDebounce() {
          ...
          if (
            Math.abs(newHeight - requestedHeight) <
            minRelevantPixelDifferenceVWC.get()
          ) {
            return;
          }
          ...
          if (isDevelopment) {
            console.log(
              `useMinHeights detected ${key} actually has min height ${newHeight}`
            );
          }
