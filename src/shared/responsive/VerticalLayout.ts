...
export const prepareVerticalLayout = <K extends string>(
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    for (let j = 0; j < priorityKeys.length; j++) {
      ...
      if (scaling.end === undefined) {
        if (
          highestBoundedPriority !== undefined &&
          priority < highestBoundedPriority
        ) {
          throw new Error(
  ...
  const sortedBoundedPriorities = Array.from(boundedPrioritiesSet).sort(
    (a, b) => a - b
  );
  const finitePriorities: ComputedVerticalLayout<K>['finitePriorities'] = [];

  const currentItemHeights = new Map() as Omit<Map<K, number>, 'get'> & {
    get: (key: K) => number;
  };
  for (let i = 0; i < keys.length; i++) {
  ...
  for (
    let priorityIndex = 0;
    priorityIndex < sortedBoundedPriorities.length;
    priorityIndex++
  ) {
    ...
    for (let i = 0; i < keys.length; i++) {
      ...
      if (scaling === undefined) {
        heights[keys[i]] = {
          type: 'fixed',
          value: currentItemHeights.get(keys[i]),
        };
        continue;
      }
      ...
      heights[keys[i]] = {
        type: 'proportional',
        range: [currentHeight, scaling.end],
      };
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    unboundedHeights[keys[i]] = {
      type: 'proportional',
      start: currentItemHeights.get(keys[i]),
    };
...
export const applyVerticalLayout = <K extends string>(
  ...
  const boundedIndex = config.finitePriorities.findIndex(
    (p) => p.endHeight > height
  );
  if (boundedIndex < 0) {
    const extraSpace = height - config.unbounded.startHeight;
    const heightPerProportional =
      extraSpace * config.unbounded.pxPerProportional;
    for (let i = 0; i < keys.length; i++) {
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (info.type === 'fixed') {
      ...
      result[keys[i]] =
        info.range[0] + (info.range[1] - info.range[0]) * progress;
    }
  }
...
function checkApplyResult<K extends string>(
  keys: K[],
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (typeof val !== 'number') {
      throw new Error(`applyVerticalLayout: result.${key} is not a number`);
    }
    if (isNaN(val)) {
      throw new Error(`applyVerticalLayout: result.${key} is NaN`);
    }
    if (!isFinite(val)) {
      throw new Error(`applyVerticalLayout: result.${key} is not finite`);
    }
    if (val < 0) {
      throw new Error(`applyVerticalLayout: result.${key} is less than 0`);
    }
  }
}
...
export const updateForNewMinHeights = <K extends string>(
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (newMinHeight < item.minHeight) {
      throw new Error(
        `updateForNewMinHeights: new min height for ${key} is less than expected`
      );
    }
    ...
    const priorityKeys = Object.keys(item.scaling);
    const sortedPriorities = priorityKeys
      .map((k) => parseInt(k))
      .sort((a, b) => a - b);
    ...
    for (
      let priorityIndex = 0;
      priorityIndex < sortedPriorities.length;
      priorityIndex++
    ) {
