...
export const useVerticalLayout = <K extends string>(
  ...
  const preparedBase = useMappedValueWithCallbacks(
    config,
    prepareVerticalLayout
  );
  ...
  if (requestedRef.current === null) {
    ...
    for (const key of keysRef.current) {
      requestedRef.current[key] = createWritableValueWithCallbacks(
        appliedBase[key]
      );
    }
  }
  ...
  const prepared = useMappedValueWithCallbacks(
    updatedConfig,
    prepareVerticalLayout
  );
  ...
  const applied = useMappedValuesWithCallbacks(
    ...
    () =>
      applyVerticalLayout(
        ...
        scrollingRequired.get()
          ? preparedBase.get().requiredHeight
          : height.get()
      ),
    {
      outputEqualityFn: (a, b) =>
        a === b ||
        keysRef.current.every((key) => Math.abs(a[key] - b[key]) < 0.1),
    }
  );
