...
import { styles } from './SurveyCheckboxGroupStyles';
...
export type SurveyCheckboxGroupProps<T extends string> = {
  ...
  choices:
    | SurveyCheckboxGroupChoice<T>[]
    | readonly SurveyCheckboxGroupChoice<T>[];
...
export const SurveyCheckboxGroup = <T extends string>({
  ...
  return (
    <View style={styles.container}>
      {choices.map(({ slug, element }, idx) => (
        <View
          style={idx !== 0 ? styles.itemNotFirstChild : undefined}
          key={slug}
        >
          <WrappedCheckbox
            checked={checked}
