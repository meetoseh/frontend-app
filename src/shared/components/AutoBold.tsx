 ...
 * text surrounded with the deliminator with font-weight 700 (or `bolder` if
 * specified)
 */
    ...
    extractParts(message, deliminators, (text, deliminator, idx) => {
      if (deliminator) {
        const bolderElement = bolder ? (
          ...
          <Text style={{ fontFamily: 'OpenSans-Bold' }} key={idx}>
            {text}
          </Text>
        );
        result.push(bolderElement);
      } else {
        result.push(<Text key={idx}>{text}</Text>);
      }
    });
    return result;
...
export const extractParts = (
  ...
  while (idx < message.length) {
    ...
    const matchingIndex = deliminators.findIndex((d) =>
      message.startsWith(d, idx)
    );
    if (matchingIndex < 0) {
      idx++;
      continue;
