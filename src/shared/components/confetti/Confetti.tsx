...
export const Confetti = ({
  ...
  const physicsVWC = useWritableValueWithCallbacks<ConfettiPhysics | null>(
    () => null
  );

  useEffect(() => {
    const physics = new ConfettiPhysics(wind, box, capacity);
    let spawningPromise: CancelablePromise<void> | null =
      physics.autoSpawn(spawnInfo);
  ...
  return (
    <RenderGuardedComponent
      props={physicsVWC}
      component={(physics) =>
        physics === null ? <></> : <ConfettiRenderer physics={physics} />
      }
    />
  );
};
