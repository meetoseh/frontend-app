...
export class ConfettiPhysics {
  ...
  autoSpawn({
    ...
    spawnChance,
    spawnChanceVelocity,
    /** Circles don't seem to work well on android */
    allowCircles,
  }: {
    position: {
      ...
      body: async (state, resolve, reject) => {
        ...
        const spawn = (at: number) => {
          ...
          for (let i = 0; i < spawns; i++) {
            ...
            const isCircle = allowCircles && Math.random() < 0.3;
            ...
            const accelerationX = sample(
              acceleration.x.min,
              acceleration.x.max
            );
            const accelerationY = sample(
              acceleration.y.min,
              acceleration.y.max
            );
            const velocityRotationX = sample(
              rotationVelocity.x.min,
              rotationVelocity.x.max
            );
            const velocityRotationY = sample(
              rotationVelocity.y.min,
              rotationVelocity.y.max
            );
            const velocityRotationZ = sample(
              rotationVelocity.z.min,
              rotationVelocity.z.max
            );
            me.spawnConfetti(
              width,
              height,
