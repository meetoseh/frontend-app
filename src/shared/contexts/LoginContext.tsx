...
const areUserAttributesEqual = (
  ...
  a.featureFlags.size === b.featureFlags.size &&
  Array.from(a.featureFlags).every((f) => b.featureFlags.has(f));
...
const defaultProps: LoginContextValue = {
  value: {
    get() {
      throw new Error(
        'attempt to access LoginContextValue defaultProps value.get'
      );
    },
    get callbacks(): Callbacks<undefined> {
      throw new Error(
        'attempt to access LoginContextValue defaultProps value.callbacks'
      );
    },
  },
  setAuthTokens: async () => {
    throw new Error(
      'attempt to setAuthTokens on LoginContextValue defaultProps'
    );
  },
...
type LoginContextQueueItemSetAuthTokens = {
  ...
  ) => TokenResponseConfig | null | undefined;
  /**
...
const acquireGlobalLoginStateAndStorageLock = (): CancelablePromise<{
  release: () => void;
}> => {
...
export const LoginProvider = ({
  ...
  useEffect(() => {
    ...
    async function handleLoginStateForever() {
      ...
      while (true) {
        ...
        if ((await handleLoggedOut()) === 'continue') {
          ifDev(() =>
            console.log(`${__logId}: continue after handleLoggedOut`)
          );
          continue;
        }
    ...
    async function checkQueue(): Promise<'continue' | 'fallthrough'> {
      ...
      const inactive = waitForValueWithCallbacksConditionCancelable(
        active,
        (v) => !v
      );
      inactive.promise.catch(() => {});
      ...
      ifDev(() =>
        console.log(
          `${__logId}: check queue waiting for canceled or global lock acquired`
        )
      );
      await Promise.race([
        inactive.promise,
        stateLocked.promise.catch(() => {}),
      ]);
      ...
      try {
        ...
        if (e instanceof Error && e.message === 'canceled') {
          if (!active.get()) {
            ifDev(() =>
              console.log(`${__logId}: check queue canceled normally`)
            );
            return 'continue';
          } else {
      ...
      try {
        ...
        switch (item.type) {
          case 'setAuthTokens':
            try {
              ifDev(() =>
                console.debug(`${__logId}: processing setAuthTokens message`)
              );
              const newTokens = item.authTokens(valueVWC.get());
              if (newTokens === undefined) {
                ifDev(() =>
                  console.debug(
                    `${__logId}: skipping setAuthTokens (returned undefined)`
                  )
                );
                break;
              }
              ...
              ifDev(() =>
                console.debug(`${__logId}: logging in via provided tokens`)
              );
          ...
          case 'setUserAttributes':
            try {
              if (oldValue.state !== 'logged-in') {
                ifDev(() =>
                  console.debug(
                    `${__logId}: skipping setUserAttributes (not logged in)`
                  )
                );
                break;
              }
              ...
              ifDev(() =>
                console.debug(
                  `${__logId}: processing setUserAttributes message`
                )
              );
              ...
              if (newAttributes === undefined) {
                ifDev(() =>
                  console.debug(
                    `${__logId}: skipping setUserAttributes (returned undefined)`
                  )
                );
                break;
              }
          ...
          default:
            ifDev(() =>
              console.warn(`${__logId}: unknown queue item type ${item}`)
            );
            break;
        }
        return 'continue';
        ...
        ifDev(() =>
          console.log(`${__logId}: check queue releasing global lock`)
        );
        lockResult.release();
      }
    }
    ...
    async function handleLoading(): Promise<'continue' | 'fallthrough'> {
      ...
      ifDev(() =>
        console.log(`${__logId}: handleLoading peeked a loading state`)
      );
    ...
    async function handleLoggedIn(): Promise<'continue' | 'fallthrough'> {
      ...
      const idTokenExpiresAtServer = getJwtExpiration(
        oldValue.authTokens.idToken
      );
      ...
      const inactiveCancelable = waitForValueWithCallbacksConditionCancelable(
        active,
        (v) => !v
      );
      ...
      ifDev(() =>
        console.log(
          `${__logId}: detected need to refresh, queue changed, or effect canceled`
        )
      );
    ...
    async function handleLoggedOut(): Promise<'continue' | 'fallthrough'> {
      ...
      ifDev(() =>
        console.log(`${__logId}: detected queue item or effect canceled`)
      );
      inactive.cancel();
      queueHasItem.cancel();
    ...
    async function assignValueFromStorageRefreshingIfNecessaryWithGlobalLock() {
      ...
      try {
        ifDev(() =>
          console.log(`${__logId}: checking storage for auth tokens`)
        );
        authTokens = await retrieveAuthTokens();
        ifDev(() =>
          console.log(`${__logId}: checking storage for user attributes`)
        );
        ...
        try {
          ...
          await storeAuthTokens(null);
          ifDev(() =>
            console.log(`${__logId}: successfully cleared auth tokens`)
          );
        } catch (e2) {
          ifDev(() =>
        ...
        try {
          ifDev(() =>
            console.log(`${__logId}: trying to clear user attributes`)
          );
          await storeUserAttributes(null);
          ifDev(() =>
            console.log(`${__logId}: successfully cleared user attributes`)
          );
        } catch (e2) {
          ifDev(() =>
      ...
      ifDev(() =>
        console.log(
          `${__logId}: have stored state: consistent logged-in; checking freshness`
        )
      );
      ...
      if (isTokenFresh(authTokens, nowServerMS)) {
        ...
        if (
          ...
          ifDev(() =>
            console.log(`${__logId}: already in matching logged-in state`)
          );
        }
        return 'continue';
      }
      ...
      ifDev(() =>
        console.log(`${__logId}: tokens are stale; checking refreshability`)
      );
      if (isRefreshable(authTokens, nowServerMS)) {
        ifDev(() =>
          console.log(
            `${__logId}: tokens are refreshable; using refreshWithGlobalLock`
          )
        );
      ...
      ifDev(() =>
        console.log(`${__logId}: tokens cannot be refreshed; clearing`)
      );
      try {
        ifDev(() => console.log(`${__logId}: trying to clear auth tokens`));
        await storeAuthTokens(null);
    ...
    async function refreshWithGlobalLockInner(
      ...
      if (valueVWC.get().state === 'logged-in') {
        const tokenExpiresAtServerMS = getJwtExpiration(
          refreshableTokens.idToken
        );
        const nowServerMS = await getCurrentServerTimeMS();
      ...
      try {
        ...
        if (e.status === 429) {
          ...
          try {
            ...
            if (retryAfterStr === null) {
              ifDev(() =>
                console.log(`${__logId}: no retry-after header present`)
              );
            } else {
              const retryAfterSecondsRaw = parseFloat(retryAfterStr);
              if (retryAfterSecondsRaw < 0) {
                ifDev(() =>
                  console.log(`${__logId}: retry-after header was negative`)
                );
              } else if (retryAfterSecondsRaw > 300) {
                ifDev(() =>
                  console.log(`${__logId}: retry-after header was too large`)
                );
              } else {
                retryAfterSeconds = retryAfterSecondsRaw;
                ifDev(() =>
                  console.log(
                    `${__logId}: retry-after header was ${retryAfterSeconds} seconds`
                  )
                );
              }
            }
            ...
            ifDev(() =>
              console.error(`${__logId}: retry-after header was malformed`, e)
            );
          }
        ...
        ifDev(() =>
          console.log(
            `${__logId}: non-retryable error; clearing and switching to logged-out`
          )
        );
        try {
          ifDev(() => console.log(`${__logId}: trying to clear auth tokens`));
      ...
      ifDev(() =>
        console.log(
          `${__logId}: successfully refreshed tokens; extracting user attributes`
        )
      );
      ...
      ifDev(() =>
        console.log(
          `${__logId}: successfully extracted user attributes, storing..`
        )
      );
      try {
        ifDev(() => console.log(`${__logId}: trying to store tokens`));
      ...
      try {
        ...
        await storeUserAttributes(userAttributes);
        ifDev(() =>
          console.log(`${__logId}: successfully stored user attributes`)
        );
      } catch (e) {
        ifDev(() =>
    ...
    async function refreshWithGlobalLock(
      refreshableTokens: TokenResponseConfig
    ): Promise<void> {
      while (true) {
