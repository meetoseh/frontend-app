...
export const LoginProvider = ({
  ...
  useEffect(() => {
    ...
    async function handleLoading(): Promise<'continue' | 'fallthrough'> {
      ...
      ifDev(() =>
        console.log(
          `${__logId}: handleLoading peeked a loading state, acquiring global lock`
        )
      );
      const inactive = waitForValueWithCallbacksConditionCancelable(
        active,
      ...
      while (true) {
        ...
        await Promise.race([
          inactive.promise,
          stateNotLoading.promise,
          stateUnlocked.promise,
        ]);
        stateUnlocked.cancel();
      ...
      try {
        ...
        if (isTokenFresh(authTokens, nowServerMS)) {
          ifDev(() =>
            console.log(`${__logId}: tokens are fresh; switching to logged-in`)
          );
          setVWC(
            valueVWC,
            {
        ...
        ifDev(() =>
          console.warn(`${__logId}: error processing loading state`, e)
        );
        return 'continue';
      } finally {
        globalLoginStateLockedVWC.set(false);
    ...
    async function handleLoggedIn(): Promise<'continue' | 'fallthrough'> {
      ...
      ifDev(() =>
        console.log(
          `${__logId}: waiting for need to refresh, queue changed, or effect canceled`
        )
      );
      await Promise.race([
      ...
      ifDev(() =>
        console.log(
          `${__logId}: detected need to refresh, trying to acquire global lock`
        )
      );
      while (true) {
        const stateUnlocked = waitForValueWithCallbacksConditionCancelable(
      ...
      try {
        ...
        ifDev(() =>
          console.error(`${__logId}: error processing logged-in state`, e)
        );
        return 'continue';
      } finally {
        globalLoginStateLockedVWC.set(false);
    ...
    async function handleLoggedOut(): Promise<'continue' | 'fallthrough'> {
      ...
      ifDev(() =>
        console.log(`${__logId}: waiting for a queue item or effect canceled`)
      );
