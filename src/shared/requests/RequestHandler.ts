...
> {
   ...
   */
  private readonly getDataFromRef: (
    ref: RefT
  ) => CancelablePromise<Result<DataT>>;
  /**
  ...
  private staleDataByRefUid: LeastRecentlyUsedCache<
    string,
    InternalData<RefT, DataT>
  >;
  ...
  constructor({
    ...
    this.log =
      this.logConfig.logging === 'none' ? () => {} : this.log.bind(this);
    this.logNest =
      this.logConfig.logging === 'none' ? () => {} : this.logNest.bind(this);
    this.logPop =
      this.logConfig.logging === 'none' ? () => {} : this.logPop.bind(this);
  }
  ...
  private log(rawMessage: string | (() => string)) {
    ...
    if (this.logBuffer === null) {
      throw new Error(
        'logBuffer should not be null if logging is set to buffer'
      );
    }
    ...
    while (
      bfr.buffer.length > bfr.maxLines ||
      bfr.sumCharacters > bfr.maxCharacters
    ) {
  ...
  private releaseLock(internalUid: string, ref: RefT) {
    ...
    try {
      ...
      if (data.locks.size > 0) {
        this.log(
          'still have other locks on data, so progressing released data'
        );
  ...
  private ensureAndLock(
    ...
    try {
        ...
        this.log(
          () =>
            `in cached, resurrecting. latest is null: ${
              ...
              cached.latest?.data.type
            }. active request is null? ${cached.activeRequest === null}`
        );
      ...
      this.log(
        'not in live or cached, initializing with no latest or active request'
      );
  ...
  private notifyLockHolders(
    ...
    try {
      ...
      this.log(
        () => `notified ${numNotified} lock holders (${numNotifySkip} skipped)`
      );
    } finally {
      this.logPop();
    }
   ...
   */
  private findBestRefHolder(
    refUid: string
  ): InternalWithRef<DataT, RefT> | null {
    ...
    try {
      ...
      while (!next.done) {
        ...
        if (internal === undefined) {
          log(
            `WARNING! locks contain bad internal uid: ${internalUid} (already released?)`
          );
          next = iter.next();
          continue;
        }
        ...
        const ref =
          internal.ref.latest.value ?? internal.ref.latest.result?.data;
        if (ref === undefined) {
  ...
  private reportExpired(refUid: string) {
    ...
    try {
      ...
      if (data === undefined) {
        this.log(
          'WARNING! reportExpired on data that is not locked (probably a caller issue)'
        );
        return;
      }
    ...
    try {
      ...
      if (
        ...
        if (promiseResult.type === 'error') {
          this.log(
            'active request failed (promise was rejected, not recoverable)'
          );
        ...
        if (promiseResult.result.type === 'errorRetryable') {
          ...
          this.log(
            `retryable at ${promiseResult.result.retryAt.toLocaleString()}`
          );

          const bestRefHolder = this.findBestRefHolder(refUid);
          if (bestRefHolder === null) {
            this.log(
              'WARNING! without a ref available, treating as unrecoverable'
            );
            const res: Result<DataT> = {
              type: 'error',
              data: undefined,
        ...
        if (
          (promiseResult.result.type === 'expired' ||
            promiseResult.result.type === 'forbidden') &&
          ...
          if (bestRefHolder !== null) {
            ...
            const comparison = this.compareRefs(
              bestRef,
              data.activeRequest.ref
            );
            if (comparison < 0) {
              ...
              bestRefHolder.ref.latest.type === 'initial' ||
              bestRefHolder.ref.latest.finishedAt.getTime() <
                Date.now() - 1000 * 60 * 5
            ) {
              this.log(
      ...
      if (
        ...
        this.log(
          'latest data is available, but its not success and old (>15s) - clearing it'
        );
        data.latest = null;
      } // USING FALLTHROUGH
  ...
  private progress(internalUid: string) {
    ...
    try {
      ...
      if (
        ...
        if (res.type === 'error') {
          this.log(
            'active request for the ref failed (promise was rejected, not recoverable)'
          );
          this.releaseLatestLock(internalUid);
          internal.ref.latest = {
            type: 'fetched',
        ...
        if (res.result.type === 'errorRetryable') {
          this.log(
            'active request for the ref failed (retryable error result)'
          );
  ...
  request({
    ...
    try {
      const data = createWritableValueWithCallbacks<
        RequestResultConcrete<DataT>
      >({
        type: 'loading',
        data: undefined,
        error: undefined,
  ...
  requestWithData({
    ...
    try {
      ...
      const concreteData = createWritableValueWithCallbacks<
        RequestResultConcrete<DataT>
      >({
        type: 'success',
        data,
      ...
      if (locked === undefined) {
        throw new Error(
          'Internal error: locked is undefined despite just ensuring!'
        );
      }
      ...
      this.log(
        'replacing latest data with new data and notifying lock holders'
      );
      const now = new Date();
  ...
  private evictOrReplaceByRefUid(
    ...
    try {
      ...
      const newData =
        data === undefined
          ? { type: 'make-request' as const, data: undefined }
          : data(oldData);
      if (newData.type === 'data') {
        this.log(
