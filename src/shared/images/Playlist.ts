...
const areAspectRatiosApproximatelyEqual = (
  ...
  const relativeDifferenceWoH =
    Math.abs(widthOverHeightA - widthOverHeightB) /
    Math.max(widthOverHeightA, widthOverHeightB);
  ...
  const relativeDifferenceHoW =
    Math.abs(heightOverWidthA - heightOverWidthB) /
    Math.max(heightOverWidthA, heightOverWidthB);
...
export const selectBestItemUsingPixelRatio = ({
  ...
  if (usesSvg && playlist.items.svg) {
    const bestVectorItem = selectBestVectorItemFromItems(
      playlist.items.svg,
      logical
    );
    const want = (() => {
    ...
    return {
      item: {
        ...adjustedItem,
        width:
          Math.ceil(adjustedItem.width * preferredPixelRatio) /
          preferredPixelRatio,
        height:
          Math.ceil(adjustedItem.height * preferredPixelRatio) /
          preferredPixelRatio,
      },
