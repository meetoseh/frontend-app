...
const getDataFromRef: (
  ref: OsehImageExportRef
) => CancelablePromise<Result<OsehImageExport>> =
  createGetDataFromRefUsingSignal({
    inner: async (ref, signal) => {
      const downloaded = await downloadItem(ref.item, ref.imageFile.jwt, {
        abortSignal: signal,
      });
      return {
        imageFileUid: ref.item.uid,
        item: ref.item,
        localUrl: downloaded.localUrl,
      };
    },
    isExpired: (ref, nowServer) =>
      getJwtExpiration(ref.imageFile.jwt) < nowServer,
  });
