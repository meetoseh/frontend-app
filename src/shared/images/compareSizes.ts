...
export const compareVectorSizes = (
  ...
  if (scaleAFromWidth && scaleBFromWidth) {
    ...
    return (
      a.height * want.width * want.width * b.height -
      b.width * want.height * want.height * a.width
    );
  } else {
    // case 4: A = T.H / a.H
    //     and B = T.W / b.W
    ...
    return (
      a.width * want.height * want.height * b.width -
      b.height * want.width * want.width * a.height
    );
  }
};
...
export function reduceImageSizeExactly(size: {
  width: number;
  height: number;
}) {
