  ...
  if (props.displayWidth === null) {
    ...
    if (item !== undefined) {
      displayWidth =
        Math.ceil(
          convertLogicalWidthToPhysicalWidth(
            props.displayHeight * (item.width / item.height)
          )
        ) / xAxisPhysicalPerLogical;
    } else {
    ...
    if (item !== undefined) {
      displayHeight =
        Math.ceil(
          convertLogicalHeightToPhysicalHeight(
            props.displayWidth * (item.height / item.width)
          )
        ) / yAxisPhysicalPerLogical;
    } else {
...
const manageRequest = async (
  ...
  const boundGetPlaylist = () =>
    getPlaylist(props, privatePlaylistHandler, publicPlaylistHandler);
  ...
  const exportRef = createWritableValueWithCallbacks<OsehImageExportRef | null>(
    null
  );
  const boundGetExport = () =>
    getExport(
      ...
      usesWebp,
      usesSvg,
      (ref) => setVWC(exportRef, ref, Object.is)
    );
  const boundGetExportCropped = () =>
    getExportCropped(props, boundGetExport, imageCropHandler);
