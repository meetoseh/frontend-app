...
export const useOsehImageStateRequestHandler = ({
  ...
  const requestQueuedCallbacks = useRef<
    Callbacks<undefined>
  ...
  useEffect(() => {
    ...
    >(playlistCacheSize);
    const playlistsByImageFileUID = new RefCountedDict<
      string,
      CancelablePromise<PlaylistWithJWT>
    >((k, v) => {
      if (v.done()) {
    ...
    const imagesByURLCache = new LeastRecentlyUsedCache<
      string,
      Promise<DownloadedItem>
    >(imageCacheSize);
    const imagesByURL = new RefCountedDict<
      string,
      CancelablePromise<DownloadedItem>
    >((k, v) => {
      if (v.done()) {
    ...
    const cropsByCropIDCache = new LeastRecentlyUsedCache<
      CropID,
      Promise<DownloadedItem>
    >(cropCacheSize);
    const cropsByCropID = new RefCountedDict<
      CropID,
      CancelablePromise<DownloadedItem>
    >((k, v) => {
      if (v.done()) {
    ...
    async function handleRequest(
      ...
      const isUnderspecified =
        req.props.displayWidth === null || req.props.displayHeight === null;

      if (req.props.displayWidth === null) {
        const requestedLogicalHeight = req.props.displayHeight;
        const goingToUsePhysicalHeight =
      ...
      if (req.props.displayHeight === null) {
        ...
        const goingToUsePhysicalWidth =
          bestItem.cropTo?.width ?? bestItem.item.width;
        const goingToUsePhysPixelsPerLogicalPixel =
          goingToUsePhysicalWidth / requestedLogicalWidth;
        const goingToUsePhysicalHeight =
          bestItem.cropTo?.height ?? bestItem.item.height;
        ...
        async (resolve, reject) => {
          ...
          try {
            if (props.isPublic || props.jwt === null) {
              ...
              playlist = {
                playlist: await fetchPrivatePlaylist(
                  props.uid,
                  props.jwt,
                  abortSignal
                ),
    ...
    async function getPlaylistItem(
      ...
      const imagePromise: Promise<DownloadedItem> = new Promise(
        async (resolve, reject) => {
          ...
          try {
            downloadedItem = await downloadItem(item, jwt, {
