  ...
  createGetDataFromRefUsingSignal({
    inner: async (ref, signal): Promise<OsehImageExportCropped> => {
      ...
      if (
        ...
        let srcSize = {
          width: ref.export.item.width,
          height: ref.export.item.height,
        };
        if (isVector) {
        ...
        let cropTo:
          ...
          | undefined = undefined;
        let rescaleTo: { width: number; height: number } | undefined =
          undefined;

        // requestedRatio.width / requestedRatio.height < srcRatio.width / srcRatio.height
        if (
          requestedRatio.width * srcRatio.height <
          requestedRatio.height * srcRatio.width
        ) {
        ...
        if (
          ...
          rescaleTo = {
            width: requestedPhysicalSize.width,
            height: requestedPhysicalSize.height,
          };
          resultSize = {
            width: requestedPhysicalSize.width,
            height: requestedPhysicalSize.height,
      ...
      if (isVector) {
        srcSize.width = Math.round(
          convertLogicalWidthToPhysicalWidth(srcSize.width)
        );
        srcSize.height = Math.round(
          convertLogicalHeightToPhysicalHeight(srcSize.height)
        );
      }
      ...
      if (ref.cropTo.displayWidth === null) {
        targetDisplaySize = {
          displayWidth:
            ref.cropTo.displayHeight * (srcSize.width / srcSize.height),
          displayHeight: ref.cropTo.displayHeight,
        };
      } else {
        targetDisplaySize = {
          displayWidth: ref.cropTo.displayWidth,
          displayHeight:
            ref.cropTo.displayWidth * (srcSize.height / srcSize.width),
        };
      }
