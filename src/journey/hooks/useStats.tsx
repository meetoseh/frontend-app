...
export const useStats = ({
  ...
  useEffect(() => {
    ...
    const availableStats: Map<number, AugmentedJourneyStats> = new Map(); // the stats we have fetched from the server, indexed by bin
    ...
    async function fetchStats() {
      ...
      while (active) {
        try {
          const startReqAt = performance.now();
    ...
    function easeNumberBetween(a: number | null, b: number | null, progress: number) {
      return Math.round((a ?? 0) + ((b ?? 0) - (a ?? 0)) * progress);
    }
    ...
    async function easeStats() {
      ...
      while (active && journeyTime.time.current <= journeyLobbyDurationSeconds * 1000) {
        ...
        if (availableStats.has(easeFromBin) && availableStats.has(easeToBin)) {
          const easeFromStats = availableStats.get(easeFromBin);
          const easeToStats = availableStats.get(easeToBin);

          if (
            easeFromStats === null ||
            easeToStats === null ||
            easeFromStats === undefined ||
            easeToStats === undefined
          ) {
            throw new Error('assertion error; availableStats contains null/undefined');
          }

          setStatsIfDifferent(easeStatsBetween(easeFromStats, easeToStats, progress));
          try {
        ...
        if (availableStats.has(easeFromBin)) {
          const easeFromStats = availableStats.get(easeFromBin);
          if (easeFromStats === null || easeFromStats === undefined) {
            throw new Error('assertion error; availableStats contains null/undefined');
          }

          setStatsIfDifferent(easeFromStats);
